...
def User_Tools_keys():
    ...
    if settings['buy'] == 'on':
        keyboard.insert(0, [InlineKeyboardButton("🔁 تمدید اشتراک", callback_data='upgrade'), InlineKeyboardButton("🛍 خرید اشتراک", callback_data='buy')])
    ...
    if settings['info_service'] == "on":
        for i in range(len(keyboard)):
            if InlineKeyboardButton("🍇 سرویس های من", callback_data='service') in keyboard[i]:
                keyboard[i].insert(0, InlineKeyboardButton("🔍 اطلاعات سرویس", callback_data='config'))
                break
    if settings['test'] == "on":
        keyboard.insert(1, [InlineKeyboardButton("تست رایگان 💙", callback_data='test')])
    if settings['buy-traffic'] == 'on':
        keyboard.insert(1, [InlineKeyboardButton("🔋 افزایش ترافیک", callback_data='traffic')])
                        ...
                        try:
                            ...
                            if passw in link:
                                ...
                                if (settings['buy'] == 'on') or (chat_id in seller_id):
                                    keyboard[0].insert(1, InlineKeyboardButton("🔁 تمدید اشتراک", callback_data=("UPG_" + cb)))
                                if (settings['buy-traffic'] == 'on') or (chat_id in seller_id):
                                    keyboard[1].insert(1, InlineKeyboardButton("🔋 افزایش ترافیک", callback_data=("UTGB_" + cb)))
                    ...
                    if (link not in filter_name) and (sshx.ASCII_Check(link) is True) and (sshx.Contains(link) is True):
                        ...
                        if check_seller_exist(chat_id) is True:
                            ...
                            if (settings['idpay'] == "on") and (settings['idpay_address'] != "None"):
                                keyboard.append([InlineKeyboardButton("💳 درگاه پرداخت", callback_data=cb_pd)])
...
def call_DUWPD(bot, query):
    ...
    if check_cache(chat_id) is False:
        ...
        add_cache(chat_id, "userdeposit")
        Code = uuid4().hex[0:10]
        invoice_url = f"https://coffeebede.com/{settings['idpay_address']}"
        keyboard = [
            [InlineKeyboardButton("لینک پرداخت 🔗", url=invoice_url)],
            [InlineKeyboardButton("<< Back", callback_data='UWM_' + Code)]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        cache_list = [price, "💳 درگاه پرداخت"]
        add_code_buy(chat_id, Code, "userdeposit", cache_list)
        text = f"""
💳 برای انتقال به درگاه پرداخت بر روی دکمه لینک پرداخت🔗 کلیک کنید.
داخل صفحه دونیت قهوه☕️، در فیلد اول نام کاربری یا یوزرنیم تلگرام و در فیلد دوم میزان مبلغ اشتراک را جهت پرداخت وارد کنید.


🟣 لطفا حتماااا از صفحه رسید پرداخت عکس بگیرید و همینجا داخل ربات بفرستید. 🟣
            """
        query.edit_message_text(text=text, reply_markup=reply_markup, parse_mode=enums.ParseMode.HTML)
    else:
        query.answer("دوباره تلاش کنید", show_alert=True)
...
def call_DTPB(bot, query):
    ...
    if check_cache(chat_id) is False:
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        cache_list = [GB, price, user, host, "💳 درگاه پرداخت"]
        add_code_buy(chat_id, Code, "traffic", cache_list)
        text = f"""
💳 برای انتقال به درگاه پرداخت بر روی دکمه لینک پرداخت🔗 کلیک کنید.
داخل صفحه دونیت قهوه☕️، در فیلد اول نام کاربری یا یوزرنیم تلگرام و در فیلد دوم میزان مبلغ اشتراک را جهت پرداخت وارد کنید.
...
def call_PD(bot, query):
    ...
    if check_cache(chat_id) is False:
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        cache_list = [days, GB, client, price, query.message.chat.first_name, UNAME, Selected_host, "💳 درگاه پرداخت"]
        add_code_buy(chat_id, Code, "add", cache_list)
        text = f"""
💳 برای انتقال به درگاه پرداخت بر روی دکمه لینک پرداخت🔗 کلیک کنید.
داخل صفحه دونیت قهوه☕️، در فیلد اول نام کاربری یا یوزرنیم تلگرام و در فیلد دوم میزان مبلغ اشتراک را جهت پرداخت وارد کنید.
...
def call_UPIDP(bot, query):
    ...
    if check_cache(chat_id) is False:
        ...
        add_cache(chat_id, "upgrade")
        reply_markup = InlineKeyboardMarkup(keyboard)
        cache_list = [days, GB, connection_limit, price, user, host, "💳 درگاه پرداخت"]
        add_code_buy(chat_id, Code, "upgrade", cache_list)
        text = f"""
💳 برای انتقال به درگاه پرداخت بر روی دکمه لینک پرداخت🔗 کلیک کنید.
داخل صفحه دونیت قهوه☕️، در فیلد اول نام کاربری یا یوزرنیم تلگرام و در فیلد دوم میزان مبلغ اشتراک را جهت پرداخت وارد کنید.
...
def call_ZBSHP(bot, query):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("💳 درگاه پرداخت", callback_data='idpay'), InlineKeyboardButton("⚪️ درگاه نکست پی", callback_data='nextpay')],
